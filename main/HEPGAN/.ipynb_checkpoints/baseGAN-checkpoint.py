...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            ...
            flat_dim = self.image_encoder(dummy).shape[1]
    ...
    def forward(self, img):
        ...
        prob = self.classifier(img_encoded)
...
z_codings = torch.rand(batch_size, 256, 1, 1).to(device)
...
if load:
    load_path = "models/class_gan_model_0905_2128.pt" # Path to load your model
    ...
    for epoch in range(n_epochs):
        for i, (real_image, real_features, flipped_image, flipped_features) in enumerate(dataloader):
            ...
            if i % 3 == 0:
                ...
                real_flipped_pred = discriminator(real_flipped_img)
                fake_pred = discriminator(fake_img)
                fake_flipped_pred = discriminator(fake_flipped_img)
            ...
            if i % 1 == 0:
                ...
                # Total generator loss is just BCE loss
                alpha = .4
    
                g_loss = (alpha*validity_loss)
        ...
        if g_loss.item() < best_g_loss:
            ...
            save_path = f"models/best_base_generator_{n_epochs}.pt"
            torch.save({
        ...
        if d_loss.item() < best_d_loss:
            ...
            timestamp = datetime.now().strftime("%m%d_%H%M")
            save_path = f"models/best_base_discriminator_{n_epochs}.pt"
            torch.save({
                "generator_state_dict": generator.state_dict(),
                "discriminator_state_dict": discriminator.state_dict(),
                "g_losses": g_losses,
        ...
        if epoch % 10 == 0:
            ...
            save_path = f"models/base_gan_model_temp.pt"
            torch.save({
                "generator_state_dict": generator.state_dict(),
                "discriminator_state_dict": discriminator.state_dict(),
                "g_losses": g_losses,
