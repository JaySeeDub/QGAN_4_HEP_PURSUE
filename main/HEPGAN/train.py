...
def train(
    ...
    # initialize component models, schedulers, and optimizers
    ...
    # grid map for computing image stats
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            # =====================================================
            #                STATISTICS LOSS (Adaptive Component Scaling)
            # =====================================================
            ...
            # individual components are scaled by their exponential moving averages
            # compute raw losses (no scaling)
            raw_dR_mean  = nn.L1Loss()(real_dR_mean,  fake_dR_mean)
            raw_dR_std   = nn.L1Loss()(real_dR_std,   fake_dR_std)
            raw_pix_mean = nn.L1Loss()(real_pixel_mean, fake_pixel_mean)
            raw_pix_std  = nn.L1Loss()(real_pixel_std,  fake_pixel_std)
            
            # update EMA scale factors
            ema_dR_mean  = ema_beta * ema_dR_mean  + (1-ema_beta) * raw_dR_mean.detach()
            ema_dR_std   = ema_beta * ema_dR_std   + (1-ema_beta) * raw_dR_std.detach()
            ema_pix_mean = ema_beta * ema_pix_mean + (1-ema_beta) * raw_pix_mean.detach()
            ema_pix_std  = ema_beta * ema_pix_std  + (1-ema_beta) * raw_pix_std.detach()
            
            # scale each term to equalize its contribution
            loss_dR_mean  = raw_dR_mean  / (ema_dR_mean  + 1e-8)
            loss_dR_std   = raw_dR_std   / (ema_dR_std   + 1e-8)
            loss_pix_mean = raw_pix_mean / (ema_pix_mean + 1e-8)
            loss_pix_std  = raw_pix_std  / (ema_pix_std  + 1e-8)
            ...
            nnz_loss = chi * nnz_loss
            
            g_loss = validity_loss + stat_loss + nnz_loss
        ...
        #   PRINT EPOCH SUMMARY
        # =====================================================
        print(f"[Epoch {epoch+1}/{n_epochs}] [Avg D loss: {avg_d:.4f}] [Avg G loss: {avg_g:.4f}]")
        
        plot_generated_samples(generator, sampleloader, batch_size=min(16, batch_size))
        
        # =====================================================
        #   EPOCH-LEVEL DEBUG PRINT
        # =====================================================
        _debug_print(
            "\n========== EPOCH STAT BREAKDOWN =========="
            f"\nAvg validity_loss   : {avg_validity:.4f}"
            f"\nAvg stat_loss       : {avg_stat:.4f}"
            f"\n  avg_dR_mean       : {(avg_loss_dR_mean * beta / 4):.4f}"
            ...
            f"\nLast batch validity_loss: {alpha * validity_loss.item():.4f}"
            f"\nLast batch stat_loss:     {beta * stat_loss.item():.4f}"
            f"\n  dR_mean:   {(loss_dR_mean.item() * beta / 4):.4f}"
