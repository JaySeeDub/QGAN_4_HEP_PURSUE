...
if True:
    for epoch in range(n_epochs):
        ...
        if g_loss.item() < best_g_loss:
            ...
            save_path = f"models/best_lossless_generator_{n_epochs}.pt"
            torch.save({
        ...
        if d_loss.item() < best_d_loss:
            ...
            timestamp = datetime.now().strftime("%m%d_%H%M")
            save_path = f"models/best_lossless_discriminator_{n_epochs}.pt"
            torch.save({
                "generator_state_dict": generator.state_dict(),
                "discriminator_state_dict": discriminator.state_dict(),
                "g_losses": g_losses,
        ...
        if epoch % 10 == 0:
            ...
            save_path = f"models/lossless_model_temp.pt"
            torch.save({
                "generator_state_dict": generator.state_dict(),
                "discriminator_state_dict": discriminator.state_dict(),
                "g_losses": g_losses,
...
if save:
    ...
    save_path = f"models/lossless_model_1_{timestamp}.pt"
    torch.save({
        "generator_state_dict": generator.state_dict(),
        "discriminator_state_dict": discriminator.state_dict(),
        "g_losses": g_losses,
