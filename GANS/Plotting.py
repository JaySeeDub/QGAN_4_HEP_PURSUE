...
def plot_smote_samples(generator, dataset, sampleloader, batch_size=16, latent_dim=256):
    # Latent vectors
    # Should be very easy to modify which values are passed as codings
    z_codings = next(iter(sampleloader)).to("cuda")
    z_codings = z_codings.to(dtype=torch.float32, device="cuda")
    z_feat = z_codings
